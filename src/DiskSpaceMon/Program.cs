...
// of the host is built -- 'DiskSpaceMon uninstall' has to work even when settings.xml is the reason
...
builder.Configuration
    ...
    .AddXmlSettingsFile(
        ...
        DiskSpaceMonSettings.CollectionPaths,
        optional: false,
        reloadOnChange: true)
    .AddEnvironmentVariables("DISKSPACEMON_");
...
builder.Services.AddSingleton<IAlertStateStore>(provider => new JsonAlertStateStore(
    provider.GetRequiredService<AppPaths>().Resolve(
        provider.GetRequiredService<IOptionsMonitor<DiskSpaceMonSettings>>()
            .CurrentValue.Alerts.StateFile),
    provider.GetRequiredService<ILogger<JsonAlertStateStore>>()));

builder.Services.AddSingleton<IValidateOptions<DiskSpaceMonSettings>, DiskSpaceMonSettingsValidator>();
builder.Services
    .AddOptions<DiskSpaceMonSettings>()
    .Bind(builder.Configuration)
    .ValidateOnStart();

if (verb == Verb.Run)
{
    builder.Services.AddHostedService<DiskSpaceMonitorWorker>();
...
catch (Exception ex) when (StartupProblem.IsSettingsProblem(ex))
{
    ...
    Console.Error.WriteLine("DiskSpaceMon cannot start.");
    ...
    Console.Error.WriteLine("  Fix it, then run 'DiskSpaceMon check' to confirm before starting the service.");
    return 2;
}

static string ExecutablePath()
    => Environment.ProcessPath ?? Path.Combine(AppContext.BaseDirectory, "DiskSpaceMon.exe");
