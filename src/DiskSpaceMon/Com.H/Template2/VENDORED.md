...
```xml
<!-- in DiskSpaceMon.csproj, replacing the Com.H.Data.Common reference -->
<PackageReference Include="Com.H.Text.Template2" Version="..." />
```

Then delete this folder. Nothing else in DiskSpaceMon changes: the namespace stays
...
is the package's one dependency. DiskSpaceMon never touches a database, but the files are copied
...
## What DiskSpaceMon actually uses

Only the value-substitution and repeat-per-row parts. The rows come from
`DiskSpaceMon.Notifications.ModelTemplateDataProvider`, an `ITemplateDataProvider` written against the
