...
/// </summary>
public sealed class DiskMonitorWorker(
    IOptionsMonitor<DiskMonSettings> options,
    DiskScanner scanner,
    IAlertStateStore stateStore,
    AlertNotifier notifier,
    TimeProvider time,
    ILogger<DiskMonitorWorker> logger) : BackgroundService
{
    private DiskMonSettings? _lastGoodSettings;
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        logger.LogInformation(
            "DiskMon started. Sweeping every {Interval}s; email {EmailState} via {Provider}.",
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            var interval = TimeSpan.FromSeconds(Math.Max(
                settings.Monitoring.IntervalSeconds, DiskMonSettingsValidator.MinimumIntervalSeconds));
        ...
        logger.LogInformation("DiskMon stopped.");
    }

    private async Task SweepAsync(DiskMonSettings settings, CancellationToken cancellationToken)
    {
    ...
    /// in effect.
    /// </remarks>
    private DiskMonSettings CurrentSettings()
    {
        try
        {
