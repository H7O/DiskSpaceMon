...
public class DiskMonSettingsValidatorTests
{
    private static ValidateOptionsResult Validate(Action<DiskMonSettings> configure)
    {
        var settings = Valid();
        configure(settings);
        return new DiskMonSettingsValidator().Validate(null, settings);
    }

    private static DiskMonSettings Valid() => new()
    {
        Monitoring =
        {
            IntervalSeconds = 300,
