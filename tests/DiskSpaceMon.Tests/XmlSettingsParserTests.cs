...
public class XmlSettingsParserTests
{
    ...
    {
        var data = Parse("<diskspacemon><monitoring><intervalSeconds>60</intervalSeconds></monitoring></diskspacemon>");

        Assert.Equal("60", data["monitoring:intervalSeconds"]);
        Assert.DoesNotContain(data.Keys, key => key.StartsWith("diskspacemon", StringComparison.OrdinalIgnoreCase));
    }
    ...
    public void AnEnvironmentVariableOverridesTheFile()
    {
        ...
        const string variable = "DISKSPACEMONTEST_email__smtp__password";
        ...
        try
        {
            var configuration = new ConfigurationBuilder()
                .AddXmlSettingsFile(
                    SettingsFile.FilePath, DiskSpaceMonSettings.CollectionPaths, optional: false)
                .AddEnvironmentVariables("DISKSPACEMONTEST_")
                .Build();
    ...
    private static DiskSpaceMonSettings Bind(string path)
    {
        var configuration = new ConfigurationBuilder()
            .AddXmlSettingsFile(path, DiskSpaceMonSettings.CollectionPaths, optional: false)
            .Build();

        var settings = new DiskSpaceMonSettings();
