...
public class AlertEmailComposerTests
{
    ...
    private static EmailSettings Settings() => new()
    {
        ...
        To = "ops@example.com",
        SubjectAlert = "[DiskSpaceMon] Low disk space on {{machineName}} ({{diskCount}})",
        SubjectRecovery = "[DiskSpaceMon] Disk space recovered on {{machineName}}",
        Templates =
        {
    ...
    public async Task TheSubjectIsRenderedFromItsOwnTemplate()
    {
        ...
        Assert.Equal($"[DiskSpaceMon] Low disk space on {Environment.MachineName} (1)", message.Subject);
    }
    ...
    public async Task TheRecoveryTemplateRendersTheSameWay()
    {
        ...
        Assert.Equal($"[DiskSpaceMon] Disk space recovered on {Environment.MachineName}", message.Subject);
    ...
    {
        var settings = Settings();
        settings.SubjectAlert = "[DiskSpaceMon]\n  Low space on {{machineName}}";
        ...
        Assert.Equal($"[DiskSpaceMon] Low space on {Environment.MachineName}", message.Subject);
    }
